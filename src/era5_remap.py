def era5_remap(var,type):   # Uncompressor as a defined function
    ...
    * Reduced Gaussian grid
        ...
        ERA5 is a ~31 km global data set of many 2-D and 3-D variables spanning decades. 
        The native data output is on the **N320** reduced Gaussian grid. On a regular 
        latitude-longitude grid, there would be 1280 grid cells in longitude (9/32˚ 
        spacing), 640 in latitude (Gaussian grid spacing of ~0.28˚, slightly closer 
        spacing near the poles, wider at the equator). The reduced grid thins points on 
        each of the 640 latitude circles to maintain approximately 32 km grid spacing, 
        also saving some storage space. As a result, each global 2-D horizontal grid is 
        stored as 1-D arrays on a dimension called `rgrid` of length 542080, saving more 
        than 1/3 of the space. Atmospheric 3-D fields on pressure levels (not the complete 
        set available) can be found at: `/shared/land/ERA5/daily/global_3d/`
     
        Complete georegistration data to remap these fields is not contained in the original 
        data files. Only the Gaussian latitudes are included in the ERA5 data files - the 
        corresponding longitudes for each point are not. There is a table with this 
        information (see: https://confluence.ecmwf.int/display/USS/Gaussian+grid+number+320 
        or the comma-sparated-value file `/shared/land/ERA5/Reduced_Gaussian.csv`), where 
        the first point in each *row* at a given Gaussian latitude is always located at 
        longitude 0˚E. Such data can be plotted directly in Python using, for example, the 
        `matplotlib.pyplot` functions `scatter` or `hexbin`, but the gaps may show at higher 
        latiudes, depending on the projection. The function below uses the *csv* file above 
        to reconstitute the full Gaussian grid using a nearest-neighbor approach along each 
        latitude band.
     
    * Land-only data
     
        There is also a great deal of 2-D daily global ERA5 data processed to retain only 
        fields over over land (excluding Antarctica) at: `/shared/land/ERA5/daily/global/`. 
        To save space, the data have been stored as 1-D series only over land points on the 
        original N320 reduced Gaussian grid (~31 km resolution). This is like what was done 
        for GSWP2, and is jokingly referred to as "dehydrated" since all the water has been 
        removed. The files `compressed_sfc_fc_daily_*.nc4` each contain one month's worth of 
        daily output for 36 variables plus static fields of latitude and longitude for each 
        grid cell. These data are saved as 1-D arrays on a dimension called `lgrid` of length 
        141780, a compression of nearly 83% over the full global grid.
     
        As with the global reduced Gaussian data described above, there is also a separate 
        utility file for the land-only data: `/shared/land/ERA5/land_index.nc`, which is a 2-D 
        array representing the original georegistered grid: 1280 grid cells in longitude and 
        534 cells in latitude (60˚S to the North Pole). The file `land_index.nc` is used to 
        "uncompress" the other data by remapping the contents of each 1-D array element to one 
        or more cells on the 2-D grid.
     
    ----------
    
    * Caveats:
    
        The function will not work on an entrire *xarray* Dataset, only on individual DataArrays 
        (i.e., one variable at a time).
    ...
    * Performance
        ...
        On **atlas3** it takes about 1/4 second to uncompress one land-only 1-D array into a 2-D 
        *xarray* DataArray. To uncompress a variable for an entire month takes about 10 seconds.
